...
include 'core/settings.template.php';
...
if (!isset($_GET['only']) || $_GET['only'] == 'TABLES') {
	...
	$tbl = var_export($TABLES, true);
	$tbl = preg_replace("#=>\n\s+#", "=> ", $tbl);
	$tbl = preg_replace("#'actions'\s*=>\s*array\s*\(\s*0\s*=>\s*'edit',\s*1\s*=>\s*'new',\s*2\s*=>\s*'view',\s*3\s*=>\s*'list',\s*4\s*=>\s*'delete',\s*5\s*=>\s*'link',\s*\)#",
		"'actions' => \$ALL_ACTIONS", $tbl);
	//special case for countries -> country
	$tbl = preg_replace("#'item_name' => '([\w\s]+)ies'#", "'item_name' => '$1y'", $tbl);
	$tbl = preg_replace("#'item_name' => '([\w\s]+)s'#", "'item_name' => '$1'", $tbl);
	//When several words separated by space
	//special case for countries -> country
	$tbl = preg_replace("#'item_name' => '([\w\s]+)ies(\s+[\w\s]+)'#", "'item_name' => '$1y$2'", $tbl);
	$tbl = preg_replace("#'item_name' => '([\w\s]+)s(\s+[\w\s]+)'#", "'item_name' => '$1$2'", $tbl);
	//
	$tbl = preg_replace("#'item_name' => '([\w\s]+)\s{2}([\w\s]+)'#", "'item_name' => '$1<->$2'", $tbl);

	$__arr = [];
	$strings_l = explode("\n", $tbl);
	foreach ($strings_l as $__i => $strcur) {
		if (preg_match("#'item_name' => '([<->\w\s]+)'#", $strcur, $_matches)) {
			$__arr[$_matches[1]] = $_matches[1];
			$strings_l[$__i] = preg_replace("#'item_name' => '([<->\w\s]+)'#", "'item_name' => \$__arr['$1']", $strcur);
		}
		if (preg_match("#'label' => '([<->\w\s]+)'#", $strcur, $_matches)) {
			$__arr[$_matches[1]] = $_matches[1];
			$strings_l[$__i] = preg_replace("#'label' => '([<->\w\s]+)'#", "'label' => \$__arr['$1']", $strcur);
		}
		if (preg_match("#'display_name' => '([<->\w\s]+)'#", $strcur, $_matches)) {
			$__arr[$_matches[1]] = $_matches[1];
			$strings_l[$__i] = preg_replace("#'display_name' => '([<->\w\s]+)'#", "'display_name' => \$__arr['$1']", $strcur);
		}

	}
	echo '$__arr = ';
	echo(var_export($__arr, true));
	echo ';', PHP_EOL;
	echo '$TABLES = ';
	echo(implode("\n", $strings_l));
	//
